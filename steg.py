...
def encrypt_and_save():
    global img, msg, encrypt_password_entry, decrypt_password_entry, decrypt_button
    img = open_image()
    if img is not None:
        msg = msg_entry.get()
        ...
        # Hide encryption widgets
        msg_label.pack_forget()
        msg_entry.pack_forget()
        encrypt_password_label.pack_forget()
        encrypt_password_entry.pack_forget()
        encrypt_button.pack_forget()
        
        # Show decryption widgets
...
encrypt_password_label.pack(pady=5)
